\documentclass[cmu]{resume}

\begin{document}
    \name{Laurence JIN}
    \phone{+852 9516-4162}
    \email{laurencj@andrew.cmu.edu}
    \linkedin{www.linkedin.com/in/laurence-jin}
    \github{github.com/laurence1126}
    \address{160 Water Street, New York, NY 10038}
    \maketitle
    %--------------------------------------------------------------------------%
    \section{EDUCATION}
    % Carnegie Mellon University
    \institution{Carnegie Mellon University, Tepper School of Business}{New York, NY}
    \position{M.Sc. in Computational Finance - MSCF}[GRE Quant: 170 / 170]{8/24 - 12/25}
    \begin{itemize}
        \item Upcoming Coursework: Stochastic Calculus, Machine Learning, Financial Computing (Python, C++) \& Optimization, Simulation Methods for Option Pricing
    \end{itemize}

    % The Chinese University of Hong Kong
    \institution{The Chinese University of Hong Kong}{Shatin, Hong Kong}
    \position{B.Sc. in Quantitative Finance and Risk Management Science}[First Class Honors]{9/19 - 7/24}
    \begin{itemize}
        \item Cumulative GPA: 3.66 / 4.00; Major GPA: 3.73 / 4.00 (Minor in Statistics)
        \item Relevant Coursework: Stochastic Modelling \& Simulations, Stochastic Processes, Statistical Computing \& Inference, Time Series, Advanced Derivatives, Data Structures \& Algorithms, Database Systems, Deep Learning
        \item Honors and Awards: Department of Statistics Scholarships (top 2 in RMSC major), Dean's List, Master's List
    \end{itemize}

    % McGill University
    \institution{McGill University}{Montreal, Quebec}
    \position{Semester Exchange Student, Faculty of Science}[Term GPA: 4.00 / 4.00]{1/23 - 5/23}
    \begin{itemize}
        \item Relevant Coursework: Matrix Numerical Analysis, Statistical Learning, Ordinary Differential Equations
    \end{itemize}
    %--------------------------------------------------------------------------%
    \section{EXPERIENCE}
    % Dymon Asia Capital (Hong Kong) Ltd.
    \institution{Dymon Asia Capital (HK) Ltd.}[Leading hedge fund in Asia with US\$5.7 billion AUM]{Central, Hong Kong}
    \position{Quantitative Research Intern, MSIF-Portfolio Management Department}{8/23 - 12/23}
    \begin{itemize}
        \item Responsible for data loading and verifying from various financial vendors, including Bloomberg, MSCI, FactSet, etc.
        \item Designed efficient SQL database schemas and optimized query performance to ensure seamless data retrieval
        \item Developed web scraping tools (Selenium) to automate data extraction; facilitated data accuracy and consistency
    \end{itemize}

    % Prudential Holdings Ltd.
    \institution{Prudential Holdings Ltd.}[Global leading insurance and asset management company]{Central, Hong Kong}
    \position{Actuarial Intern, Asset / Liability Management (Group Investment)}{5/23 - 8/23}
    % \note{Contracted with Robert Walters (HK) Ltd.}
    \begin{itemize}
        \item Generated stochastic simulation sets under risk-neutral scenarios; deeply involved in data collection, parameters refinement and testing to meet calibration targets
        \item Assisted in research and further development of modelling process, improved the workflow using Python automation
        \item Performed market-consistent valuations for multiple countries and real-world projections based on simulation sets
    \end{itemize}

    % China Galaxy Securities Co., Ltd.
    \institution{China Galaxy Securities Co., Ltd.}[Top-tier investment bank in China]{Beijing, China}
    \position{Data Analyst Intern, Securities Finance Department}{6/22 - 8/22}
    \begin{itemize}
        \item Calculated P\&L, VaR and sensitivities of several portfolios, implemented P\&L attribution analysis and backtesting
        \item Investigated on stock ratings and trading behaviors, collaborated in pricing model refinement and optimization
        \item Tracked domestic financial market movements and regulatory environment, analyzed credit market data using Python
    \end{itemize}

    % Minsheng Equity Investment Fund Management Co., Ltd.
    \institution{Minsheng Equity Investment Fund Management Co., Ltd.}{Beijing, China}
    \position{Industry Research Intern, Investment Management Department}{5/21 - 8/21}
    \begin{itemize}
        \item Conducted research and analysis on target client profiles and relevant industry (photovoltaic, autonomous vehicle)
        \item Appraised comparable listed companies valuation (EV/EBITDA, P/E, DCF) and comparable transactions valuation
        \item Assisted in drafting pitch books for potential transactions, composed marketing materials for reporting purposes
    \end{itemize}
    %--------------------------------------------------------------------------%
    \section{PROJECTS}
    % FINA 4380 Course Project
    \institution{A Quantitative Portfolio Construction Strategy}[][\href{https://github.com/laurence1126/Quant-Trading-Strategy}{\githubIcon}]{1/24 - 4/24}
    \begin{itemize}
        \item Integrated Bayesian shrinkage, Black-Litterman model and multi-factor models in re-evaluating posterior predictive moments of stock returns, reduced estimation errors by optimizing the balance between bias and variance.
        \item Backtested the portfolio performance via Backtrader and QuantStats (Python packages), achieved a Sortino ratio of 1.1 and a CAGR of 15.89\% over a four-year period
    \end{itemize}

    % FINA 4150 Course Project
    \institution{Structured Products Pricing}[Equity-Linked Note][\href{https://github.com/laurence1126/Equity-Linked-Note-Pricer}{\githubIcon}]{9/23 - 12/23}
    \begin{itemize}
        \item Derived and interpolated the Black-Scholes implied volatility surfaces with real option quotes from Bloomberg, implemented Dupire's equation and Gatheral's method to calibrate local volatility surfaces for simulation purposes
        \item Designed, restructured and optimized Python algorithms for numerical methods in matrix computations / differentiation, successfully decreased the program's calculation time by at least 80\%
    \end{itemize}

    % Student Helper (Faculty of BA)
    % \institution{CUHK Business School Postgraduate Admission Data Research}{2/22 - 8/23}
    % \begin{itemize}
    %     \item Developed and implemented data analysis utilizing  Python / Excel on CUHK Graduate School candidate profiles, processed and clustered over 250,000 data points for 5 consecutive academic years
    %     \item Visualized the data (Matplotlib, Tableau) and designed graphs \& charts for reporting analysis results
    % \end{itemize}

    % CuSell
    \institution{CuSell}[An online second-hand trading platform for CUHK students][\href{https://github.com/1155141394/CuSell}{\githubIcon}]{1/22 - 4/22}
    \begin{itemize}
        \item Scripted in HTML / CSS / JavaScript; designed and developed ornamented, user-friendly UI \& web pages
        \item Built connection and integrated front-end user-facing interfaces with back-end web server framework (Python Django)
        \item Regularly maintained, managed, and updated project codes on GitHub with the latest dependencies
    \end{itemize}
    %-------------------------------------------------------------------------%
    \section{ADDITIONAL INFORMATION}
    \ifthenelse{\equal{\resumeSytle}{cmu}}{\vspace{\vspaceInstitution}}{}
    \begin{itemize}
        \item \skills{Software}{Experienced with Visual Studio Code, GitHub, Bloomberg, Wind, Microsoft Azure, SSMS}
        \item \skills{Programming}{Python (OOP, data analysis \& visualization), R, MATLAB, Excel VBA, Java, C, C++, Linux Command}
        \item \skills{Database Management}{Knowledgeable in DBMS (SQL Server, MySQL, Microsoft Access, etc.)}
        % \item \skills{Language}{Fluent in English (IELTS 7.5), Mandarin (native), Cantonese (basic)}
    \end{itemize}
    %-------------------------------------------------------------------------%
\end{document}
