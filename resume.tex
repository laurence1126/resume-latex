...
\begin{document}
    ...
    \position{Master of Science in Computational Finance - MSCF}[QPA: 4.04 / 4.33]{12/25}
    \begin{itemize}
    ...
    \position{Semester Exchange Student, Mathematics \& Statistics}[Term GPA: 4.00 / 4.00]{05/23}
    \begin{itemize}
    ...
    \section{PROJECTS}
    % Machine Learning Capstone with BNY
    \institution{Optimal-Transport Regime Detection for Systematic Trading}[][\githubLink{https://github.com/laurence1126/Wasserstein-Regime-Clustering}]{08/25 - 12/25}
    \begin{itemize}
        \item Built a robust non-parametric regime discovery pipeline using Wasserstein K-means clustering; validated on synthetic jump-diffusion data with 94.14\% labeling accuracy and accelerated rolling re-fits via warm-started centroids
        \item Constructed a regime-conditioned growth/defensive rotation signal from WK-means clustering of SPX hourly returns
        \item Conducted out-of-sample backtests with grid-based hyperparameter tuning; delivered a portfolio performance of 1.85 Sharpe ratio, 25.71\% annualized return, and -11.42\% maximum drawdown (Oct 2020 - Oct 2025)
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Backtested portfolio performance via Backtrader and QuantStats (Python packages); achieved a Sharpe ratio of 1.1 and a CAGR of 15.89\% over a four-year testing period (Jan 2020 - Mar 2024)
    \end{itemize}
